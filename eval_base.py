...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        ...
        seg_model = eval('networks.' + args.model + '.GFSS_Model')(
            ...
            dilated=dilated, os=stride, n_novel=args.novel_classes, is_ft=False)
        if (not engine.distributed) or (engine.distributed and engine.local_rank == 0):
        ...
        prob_path = os.path.join(save_path, 'prob')
        ...
        for seed in seeds:
            ...
            for idx, data in enumerate(test_loader):
                ...
                with torch.no_grad():
                    output = model(image)
                    output = F.interpolate(input=output, size=label.shape[-2:], mode='bilinear', align_corners=True)
                ...
                if len(label.shape) < 3:
                    ...
                    out_dict = {'outputs': output.cpu().numpy()}
                    scipy.io.savemat(os.path.join(prob_path, id[0] + ".mat"), out_dict)

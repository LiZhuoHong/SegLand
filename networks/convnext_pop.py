...
class FPN_Seg_OCR_Decoder(nn.Module):

    def __init__(self, in_ch, out_ch):
        super(FPN_Seg_OCR_Decoder, self).__init__()
    ...
    def forward(self, x):
        ...
        feats = self.norm(feats.permute(0, 2, 3, 1)).permute(0, 3, 1, 2)
        return feats
...
class GFSS_Model(nn.Module):
    ...
    def __init__(self, n_base, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        ...
        d_model = 192
        self.decoder = FPN_Seg_OCR_Decoder(sum(self.backbone.filters), d_model)

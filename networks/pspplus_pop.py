...
class PSP_Plus_Decoder(nn.Module):
    def __init__(self, features, out_features=96, reduced_feature=32, sizes=(1, 2, 3, 6), norm_layer=nn.BatchNorm2d):
        super(PSP_Plus_Decoder, self).__init__()
        self.psp = PSPModule(features, out_features, sizes, norm_layer)
        self.reduce = nn.Sequential(nn.Conv2d(out_features, reduced_feature, 1, 1, 0, 1),
                                    norm_layer(reduced_feature),
                                    nn.ReLU())
        self.fc = nn.Sequential(nn.Conv2d(out_features + reduced_feature, out_features, 3, 1, 1, 1),
                                norm_layer(out_features),
                                nn.ReLU(),
                                nn.Conv2d(out_features, out_features, 1, 1, 0, 1))
    
    def forward(self, feats):
        low = self.reduce(feats[1])
        high = F.interpolate(self.psp(feats[-1]), size=low.shape[-2:], mode='bilinear', align_corners=True)
        out = self.fc(torch.cat([low, high], dim=1))
        return out
...
class GFSS_Model(nn.Module):
    ...
    def __init__(self, n_base, criterion=None, norm_layer=nn.BatchNorm2d, use_base=True, is_ft=False, n_novel=0, **kwargs):
        ...
        self.decoder = PSP_Plus_Decoder(2048, out_features=d_model, norm_layer=norm_layer)
